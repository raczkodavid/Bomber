...
public class GameModel : IDisposable
{
    ...
    public void NewGame(Stream stream)
    {
        // The game must be ended or paused to start a new game
        if (IsGameRunning)
            return;
    ...
    private void LoadGameBoard(Stream stream)
    {
        GameBoardDto gameBoardDto = _dataAccess.LoadGameBoard(stream);
        int mapSize = gameBoardDto.MapLayout.GetLength(0);
        TileType[,] mapLayout = new TileType[mapSize, mapSize];
        for (int i = 0; i < mapSize; i++)
        {
